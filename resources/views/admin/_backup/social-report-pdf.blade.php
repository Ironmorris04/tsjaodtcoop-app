...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr style="background: #e8f4f8; font-weight: bold;">
                ...
                <td class="text-center">{{ number_format($cooperativeActivities->sum('participants_count')) }}</td>
                <td class="text-right">₱{{ number_format($cooperativeActivities->sum('amount_utilized'), 2) }}</td>
                <td colspan="2"></td>
            </tr>
        </tbody>
    </table>

    <!-- Cooperative Summary Box -->
    <div class="summary-box">
        <strong>Cooperative Activities Summary:</strong><br>
        Total Activities: {{ $cooperativeActivities->count() }} | 
        Total Participants: {{ number_format($cooperativeActivities->sum('participants_count')) }} | 
        Total Amount: ₱{{ number_format($cooperativeActivities->sum('amount_utilized'), 2) }}
    </div>
    @else
    <div class="section-title">A. For Cooperative Itself</div>
    <div class="fund-info">Source of Fund: CETF, Optional Fund or Outright Expense</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr style="background: #e8f4f8; font-weight: bold;">
                <td colspan="3" class="text-right">TOTAL:</td>
                <td class="text-center">{{ number_format($communityActivities->sum('participants_count')) }}</td>
                <td class="text-right">₱{{ number_format($communityActivities->sum('amount_utilized'), 2) }}</td>
                <td colspan="2"></td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="summary-box">
        <strong>Community Activities Summary:</strong><br>
        Total Activities: {{ $communityActivities->count() }} | 
        Total Participants: {{ number_format($communityActivities->sum('participants_count')) }} | 
        Total Amount: ₱{{ number_format($communityActivities->sum('amount_utilized'), 2) }}
    </div>
    @else
    <div class="section-title">B. For The Community</div>
    <div class="fund-info">Source of Fund: CDF or Outright Expense</div>
    ...
    <!-- Overall Summary -->
    @if($cooperativeActivities->count() > 0 || $communityActivities->count() > 0)
    <div class="summary-box" style="background: #d5e8f7; border-left-color: #2874a6;">
        <strong style="font-size: 9pt;">GRAND TOTAL SUMMARY:</strong><br>
        Total Activities: {{ $cooperativeActivities->count() + $communityActivities->count() }} | 
        Total Participants: {{ number_format($cooperativeActivities->sum('participants_count') + $communityActivities->sum('participants_count')) }} | 
        Total Amount Utilized: ₱{{ number_format($cooperativeActivities->sum('amount_utilized') + $communityActivities->sum('amount_utilized'), 2) }}
    </div>
    @endif
